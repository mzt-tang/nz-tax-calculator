...
export default function calculateTax(income) {
  ...
  for (let i = 0; i < TAX_BRACKETS.length; i++) {
    ...
    const maxTaxBracketAmounttoPay =
      (income - TAX_BRACKETS[i - 1].bracket) *
      rateToActualDecimal(TAX_BRACKETS[i].rate);
...
function calculateMaxBracketAmount(taxBracketIndex) {
  ...
  const totalIncomeForBracket =
    TAX_BRACKETS[taxBracketIndex].bracket -
    TAX_BRACKETS[taxBracketIndex - 1].bracket;
  return (
    totalIncomeForBracket *
    rateToActualDecimal(TAX_BRACKETS[taxBracketIndex].rate)
  );
}
